...
const PendingReports = () => {
  ...
  const handleActivate = async (id, disasterCategory, disasterInfo) => {
    try {
      ...
      const aiResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/ai/priority`, {
        method: "POST",
      ...
      const updateResponse = await fetch(`${process.env.REACT_APP_BASE_URL}/reports/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDelete = async (id) => {
    try {
      await fetch(`${process.env.REACT_APP_BASE_URL}/reports/${id}`, { method: "DELETE" });
